...
const StartGame = (props: PropsI) => {
  ...
  const gamePreconditions: boolean = useMemo(
    () =>
      ...
      userInput.every(
        (x) => x.userInput.nickname !== DEFAULT_USER_INPUT.NICKNAME && x.userInput.color !== DEFAULT_USER_INPUT.COLOR
      ),
    [userInput]
  )
  ...
  return (
    <Box sx={{ display: 'flex', gap: '1rem', flexDirection: 'column', alignItems: 'center' }}>
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: '.75rem' }}>
        <PlayerInputContainer title="Player 1" idPlayer="player1" setUserInput={setUserInput} userInput={userInput} />
        <PlayerInputContainer title="Player 2" idPlayer="player2" setUserInput={setUserInput} userInput={userInput} />
      </Box>
      <Box sx={{ margin: '100px' }}>
        <IconButton
          sx={{
